...
struct Color {
  ...
  const static Color Black, White, Red, Green, Blue, Magenta, Cyan, Yellow,
      Gray; // use Color::Black
};
...
void InitWindow(int &argc, char **&argv, V2 WindowSize, V2 WindowStartPos,
                std::string name);
...
// Font
void DrawStringFontMono(V2 pos, std::string text, float fontSize = 20,
                        float thickness = 3, Color c = Color::Black);
void DrawStringFontRoman(V2 pos, std::string text, float fontSize = 20,
                         float thickness = 3, Color c = Color::Black);

// Cr�e une texture � partir d'une description 1 lettre = 1 couleur / voir
// exemple
int InitTextureFromString(V2 &Size, const std::string &Sprite);

// Affiche un rectangle textur� � la position (x,y) (x+w,y+h)
